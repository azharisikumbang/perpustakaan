    ...
    <div class="mb-8">
        <div class="block sm:flex items-center md:divide-x md:divide-gray-100 mb-4">
            <form class="sm:pr-3 mb-4 sm:mb-0" action="" method="get">
                <label for="anggota-search" class="sr-only">Search</label>
            ...
            <div class="flex items-center sm:justify-end w-full">
                <div class="hidden md:flex pl-2 space-x-1">
                    ...
                    >
                        <option value="{{ secure_url(route('anggota.index', ['limit' => 10 ])) }}" <?= ($per_page == 10) ? 'selected' : '' ?>>10</option>
                        <option value="{{ secure_url(route('anggota.index', ['limit' => 20 ])) }}" <?= ($per_page == 20) ? 'selected' : '' ?>>20</option>
                        <option value="{{ secure_url(route('anggota.index', ['limit' => 50 ])) }}" <?= ($per_page == 50) ? 'selected' : '' ?>>50</option>
                        <option value="{{ secure_url(route('anggota.index', ['limit' => 100 ])) }}" <?= ($per_page == 100) ? 'selected' : '' ?>>100</option>
                    </select>
                </div>
                <a href="{{ route('anggota.create') }}" class="text-white bg-green-600 hover:bg-green-700 focus:ring-4 focus:ring-green-200 font-medium inline-flex items-center rounded-lg text-sm px-3 py-2 text-center sm:ml-auto">
                    <svg class="-ml-1 mr-2 h-6 w-6" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M10 5a1 1 0 011 1v3h3a1 1 0 110 2h-3v3a1 1 0 11-2 0v-3H6a1 1 0 110-2h3V6a1 1 0 011-1z" clip-rule="evenodd"></path></svg>
                    Tambah Data Keanggotaan
                </a>
            </div>
        </div>
        <div class="flex flex-col">
            <div class="overflow-x-auto">
                <div class="align-middle inline-block min-w-full">
                    <div class="shadow overflow-hidden"><table class="table-fixed min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-100">
                                <tr>
                                    <th class="w-4 p-4 text-left text-xs font-medium text-gray-500 uppercase">No</th>
                                    <th class="p-4 text-left text-xs font-medium text-gray-500 uppercase">Nomor Keanggotaan</th>
                                    <th class="p-4 text-left text-xs font-medium text-gray-500 uppercase">Nama</th>
                                    <th class="p-4 text-left text-xs font-medium text-gray-500 uppercase">Institusi</th>
                                    <th class="p-4 text-left text-xs font-medium text-gray-500 uppercase">Kontak</th>
                                    <th></th>
                                </tr>
                            </thead>
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse($data as $anggota)
                                <tr class="anggota-item hover:bg-gray-100">
                                    <td class="p-4 whitespace-nowrap text-sm font-normal text-gray-500">
                                        {{ $loop->iteration }}
                                    </td>
                                    <td class="p-4 whitespace-nowrap text-sm font-normal text-gray-500">
                                        <a class="hover:underline cursor-pointer" href="{{ route('anggota.show', ['anggotum' => $anggota['id']]) }}">{{ $anggota['kode'] }}</a>
                                    </td>
                                    <td class="p-4 whitespace-nowrap text-sm font-normal text-gray-500">
                                        {{ $anggota['nama'] }}
                                    </td class='p-4 whitespace-nowrap text-sm font-normal text-gray-500'>
                                    ...
                                    <td class="p-4 whitespace-nowrap" x-data="{ deleteOpen: false }">
                                        <div class="space-x-2 justify-end flex">
                                            <a href="{{ route('anggota.edit', ['anggotum' => $anggota['id']]) }}" class="text-white bg-yellow-600 hover:bg-yellow-700 focus:ring-4 focus:ring-yellow-200 font-medium rounded-lg text-sm inline-flex items-center px-3 py-2 text-center">
                                            ...
                                            <div class="relative w-full max-w-md px-4 h-full md:h-auto">
                                                <div class="bg-white rounded-lg shadow relative">
                                                    ...
                                                    <div class="p-6 pt-0 text-center">
                                                        ...
                                                        <h3 class="text-xl font-normal text-gray-500 mt-5 mb-6">Anda yakin ingin menghapus data ini ?</h3>
                                                        <button data-anggota-id="{{ $anggota['id'] }}" @click="deleteKeanggotaanItem" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-base inline-flex items-center px-3 py-2.5 text-center mr-2">
                                                            Ya, Saya Yakin
                                                        </a>
        ...
        function deleteKeanggotaanItem(e) {
            e.target.innerHTML = "Menghapus...";

            const method = "DELETE";   
            const anggotaId = e.target.dataset.anggotaId;
            const anggotaItemRowElement = e.target.closest('.anggota-item')

            axios.post(`/admin/anggota/${anggotaId}/`, { "_method": method })
                .then(response => {
                    anggotaItemRowElement.remove()
