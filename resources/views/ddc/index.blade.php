<x-app-layout>
    ...
    <div class="mb-8">
        <div class="block sm:flex items-center md:divide-x md:divide-gray-100 mb-4">
            <form class="sm:pr-3 mb-4 sm:mb-0" action="" method="get">
                <label for="ddc-search" class="sr-only">Search</label>
            ...
            <div class="flex items-center sm:justify-end w-full">
                <div class="hidden md:flex pl-2 space-x-1">
                    ...
                    >
                        <option value="{{ secure_url(route('ddc.index', ['limit' => 10 ])) }}" <?= ($per_page == 10) ? 'selected' : '' ?>>10</option>
                        <option value="{{ secure_url(route('ddc.index', ['limit' => 20 ])) }}" <?= ($per_page == 20) ? 'selected' : '' ?>>20</option>
                        <option value="{{ secure_url(route('ddc.index', ['limit' => 50 ])) }}" <?= ($per_page == 50) ? 'selected' : '' ?>>50</option>
                        ...
                        <table class="table-fixed min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-100">
                                <tr>
                                    ...
                                    <th class="p-4 text-left text-xs font-medium text-gray-500 uppercase">Kode DDC</th>
                                    <th class="p-4 text-left text-xs font-medium text-gray-500 uppercase">Nama Klasifikasi</th>
                                    <th class="p-4 text-left text-xs font-medium text-gray-500 uppercase">Jumlah</th>
                                    <th></th>
                                </tr>
                            </thead>
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse($data as $ddc)
                                <tr class="ddc-item hover:bg-gray-100">
                                    <td class="p-4 whitespace-nowrap text-sm font-normal text-gray-500">
                                        {{ $loop->iteration }}
                                    </td>
                                    <td class="p-4 whitespace-nowrap text-sm font-normal text-gray-500">
                                        <a class="hover:underline cursor-pointer" href="{{ route('ddc.show', ['ddc' => $ddc['id']]) }}">{{ $ddc['kode'] }}</a>
                                    </td>
                                    <td class="p-4 whitespace-nowrap text-sm font-normal text-gray-500">
                                        {{ $ddc['klasifikasi'] }}
                                    </td>
                                            ...
                                            <div class="relative w-full max-w-md px-4 h-full md:h-auto">
                                                <div class="bg-white rounded-lg shadow relative">
                                                    ...
                                                    <div class="p-6 pt-0 text-center">
                                                        ...
                                                        <h3 class="text-xl font-normal text-gray-500 mt-5 mb-6">Anda Yakin ingin menghapus ddc ini ?</h3>
                                                        <button data-ddc-id="{{ $ddc['id'] }}" @click="deleteddc" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-base inline-flex items-center px-3 py-2.5 text-center mr-2">
                                                            Ya, Saya Yakin
                                                        </a>
    ...
    <script type="text/javascript">
        function deleteddc(e) {
            const method = "DELETE";   
            const ddcId = e.target.dataset.ddcId;
            const ddcItemRowElement = e.target.closest('.ddc-item');

            e.target.innerHTML = "Menghapus...";
            axios.post(`/admin/ddc/${ddcId}/`, { "_method": method })
                .then(response => {
                    ddcItemRowElement.remove()
