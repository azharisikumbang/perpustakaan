<x-app-layout>
    ...
    <div class="mb-8">
        <div class="block sm:flex items-center md:divide-x md:divide-gray-100 mb-4">
            ...
            <div class="flex items-center sm:justify-end w-full">
                <div class="hidden md:flex pl-2 space-x-1">
                    ...
                    >
                        <option value="{{ secure_url(route('keanggotaan.index', ['limit' => 10 ])) }}" {{ ($per_page == 10) ? 'selected' : ''}}>10</option>
                        <option value="{{ secure_url(route('keanggotaan.index', ['limit' => 20 ])) }}" {{ ($per_page == 20) ? 'selected' : ''}}>20</option>
                        <option value="{{ secure_url(route('keanggotaan.index', ['limit' => 50 ])) }}" {{ ($per_page == 50) ? 'selected' : ''}}>50</option>
                        <option value="{{ secure_url(route('keanggotaan.index', ['limit' => 100 ])) }}" {{ ($per_page == 100) ? 'selected' : ''}}>100</option>
                    </select>
                </div>
                <a href="{{ route('keanggotaan.create') }}" class="text-white bg-green-600 hover:bg-green-700 focus:ring-4 focus:ring-green-200 font-medium inline-flex items-center rounded-lg text-sm px-3 py-2 text-center sm:ml-auto">
                    ...
                    <div class="shadow overflow-hidden">
                        <table class="table-fixed min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                <tr class="keanggotaan-item hover:bg-gray-100">
                                    <td class="p-4 whitespace-nowrap text-sm font-normal text-gray-500">
                                        {{ $loop->iteration }}
                                    </td>
                                    <td class="p-4 whitespace-nowrap text-sm font-normal text-gray-500">
                                        <a class="hover:underline cursor-pointer" href="{{ route('keanggotaan.show', ['keanggotaan' => $anggota['id']]) }}">{{ $anggota['nomor_identitas'] }}</a>
                                    </td>
                                    <td class="p-4 whitespace-nowrap text-sm font-normal text-gray-500">
                                        {{ $anggota['nama'] }}
                                    </td class='p-4 whitespace-nowrap text-sm font-normal text-gray-500'>
                                    ...
                                    <td class="p-4 whitespace-nowrap" x-data="{ deleteOpen: false }">
                                        <div class="space-x-2 justify-end flex">
                                            <a href="{{ route('keanggotaan.edit', ['keanggotaan' => $anggota['id']]) }}" class="text-white bg-yellow-600 hover:bg-yellow-700 focus:ring-4 focus:ring-yellow-200 font-medium rounded-lg text-sm inline-flex items-center px-3 py-2 text-center">
                                            ...
                                            <div class="relative w-full max-w-md px-4 h-full md:h-auto">
                                                <div class="bg-white rounded-lg shadow relative">
                                                    ...
                                                    <div class="p-6 pt-0 text-center">
                                                        ...
                                                        <button data-keanggotaan-id="{{ $anggota['id'] }}" @click="deleteKeanggotaanItem" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-base inline-flex items-center px-3 py-2.5 text-center mr-2">
                                                            Ya, Saya Yakin
                                                        </a>
        ...
        function deleteKeanggotaanItem(e) {
            ...
            const method = "DELETE";   
            const keanggotaanId = e.target.dataset.keanggotaanId;
            const keanggotaanItemRowElement = e.target.closest('.keanggotaan-item')

            axios.post(`/admin/keanggotaan/${keanggotaanId}/`, { "_method": method })
                .then(response => {
                    keanggotaanItemRowElement.remove()
